  ...
  return (
    <div className="w-full h-full py-10 max-w-[1200px] mx-auto px-5 lg:px-0">
      <div className="flex flex-col gap-y-3">
        {/* Search */}
        <div className="w-full flex gap-3 px-4 py-2  rounded-[0.3rem] border border-[rgba(128,121,121,10px)]  mt-6">
          <input
            type="search"
            placeholder="Search Note's here..."
            className="focus:outline-none w-full bg-neutral-500"
        ...
        <div className="flex flex-col border border-[rgba(128,121,121,10px)] py-4 rounded-[0.4rem] bg-">
          
          <h2 className="px-4 text-4xl font-bold border-b border-[rgba(128,121,121,5px)] pb-4">
            All Saved Note's
          </h2>
          <div className="w-full px-4 pt-4 flex flex-col gap-y-5">
            {filteredPastes.length > 0 ? (
              filteredPastes.map((paste) => (
                <div
                  key={paste?._id}
                  className="border border-[rgba(128,121,121,10px)] w-full gap-y-6 justify-between flex flex-col sm:flex-row p-4 rounded-[0.3rem]"
                >
                  {/* heading and Description */}
                  <div className="w-[50%] flex flex-col space-y-3">
                    <p className="text-4xl font-semibold ">{paste?.title}</p>
                    <p className="text-lg font-normal line-clamp-3 max-w-[80%] text-black">
                      {paste?.content}
                    </p>
                  </div>
